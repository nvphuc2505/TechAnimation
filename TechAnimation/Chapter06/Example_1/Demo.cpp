	...
	void* pUserContext)
{
	...
	g_OBBInstance1 = new OBB(XMFLOAT3(3.0f, 1.0f, 1.0f));
	g_OBBInstance2 = new OBB(XMFLOAT3(5.0f, 1.0f, 1.0f));
	...
	double fTime, float fElapsedTime, void* pUserContext)
{
	...
	g_OBBInstance1->Render();



	// D3D11_MAPPED_SUBRESOURCE mappedResource;
	world = XMLoadFloat4x4(&g_World);
...
void CALLBACK OnFrameMove(double fTime, float fElapsedTime, void* pUserContext)
{
	...
	XMMATRIX boxOffset = XMMatrixTranslation(3.0f, 3.5f, 0.0f);
	XMStoreFloat3(&g_OBBInstance2->mPosition, boxOffset.r[3]);

	XMMATRIX boxRotate = XMMatrixRotationZ(45.0f * XMConvertToRadians((float)fTime));
	XMStoreFloat4(&g_OBBInstance2->mRotation, XMQuaternionRotationMatrix(boxRotate));

	XMStoreFloat4x4(&g_World, boxRotate * boxOffset);

	if(g_OBBInstance1->Intersect(*g_OBBInstance2) || g_OBBInstance2->Intersect(*g_OBBInstance1))
		OutputDebugStringA("Intersection\n");
	else
		OutputDebugStringA("No Intersection\n");
